...
def main():
    ...
    frame_count = 0
    output_folder = f"3D_data/output_data_{formatted_time_for_filename}"
    ...
    while True:
        ...
        # 输出 JSON 文件
        if color_neck_3d is not None:  # 添加检查

            # 保存图像
            color_image_filename = os.path.join(output_folder, f"color_image_{frame_count}.png")
            transformed_color_image_filename = os.path.join(output_folder, f"transformed_color_image_{frame_count}.png")

            cv2.imwrite(color_image_filename, color_skeleton)
            cv2.imwrite(transformed_color_image_filename, transformed_color_skeleton)

            json_data = {
                "color_neck_3d": color_neck_3d.tolist(),
                "depth_neck_3d": depth_neck_3d.tolist(),
                "depth_transformed_neck_3d": depth_transformed_neck_3d,
                "body_neck_3d": neck_3d.tolist()
            }

            json_filename = os.path.join(output_folder, f"output_{frame_count}.json")
